...
Deno.serve(async (req) => {
  ...
  let body: { id?: unknown; name?: unknown; price?: unknown; unit?: unknown }
  try {
    body = await req.json()
  } catch {
    return new Response(
  ...
  // Fetch existing row to determine type (immutable)
  ...
  if (existing.type === "resource" && !unit) {
    return new Response(
  ...
  const { data, error } = await supabase
    .from("tariffs")
    .update({ name, price, unit: existing.type === "resource" ? unit : null })
    .eq("id", id)
    .select()
    .single()
